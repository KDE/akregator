...
bool BrowserFrame::Private::loadPartForMimetype(const QString &mimetype)
{
    ...
    connect(part.data(), &QObject::destroyed, this, &Private::partDestroyed, Qt::QueuedConnection);
...
{
    if (part) {
        connect(part.data(), &KParts::Part::setWindowCaption,
                q, &Frame::slotSetCaption);
        connect(part.data(), &KParts::Part::setStatusBarText,
                q, &Frame::slotSetStatusText);
        connect(part.data(), &KParts::ReadOnlyPart::started, q, &Frame::slotSetStarted);
        connect(part, SIGNAL(completed()), q, SLOT(slotSetCompleted()));
        connect(part.data(), &KParts::ReadOnlyPart::canceled,
